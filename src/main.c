...
void SPI_port_init()
{
    ...
    gpio_init(GPIOB, GPIO_MODE_OUT_OD, GPIO_OSPEED_50MHZ, NSS);
...
int8_t convert_reg_addr(int8_t addr, BOOL is_read)
{
    // addr格式为
    // 7       -       6 - 5 - 4 - 3 - 2 - 1 - 0
...
int main()
{
    ...
    printf("sending\n\r");
    uint16_t initdata = write_byte(2, PCD_RESETPHASE);
    spi_i2s_data_transmit(SPI1, convert_reg_addr(2, FALSE));
    ...
    while (1)
    {
        ...
        for(uint8_t p = DivlEnReg ; p > 0; p = 0){
            printf("waiting flag 1\n");
            while(RESET == spi_i2s_flag_get(SPI1, SPI_FLAG_TBE));
            printf("sending\n");
            sendingdata = write_byte(p, 100);
            gpio_bit_set(GPIOB, NSS);
            gpio_bit_reset(GPIOB, NSS);
            spi_i2s_data_transmit(SPI1, convert_reg_addr(p, FALSE));
            spi_i2s_data_transmit(SPI1, 100);
            printf("waiting flag 2\n");
            while(RESET == spi_i2s_flag_get(SPI1, SPI_FLAG_RBNE));
            while(RESET == spi_i2s_flag_get(SPI1, SPI_FLAG_TBE));
            printf("sending\n");
            sendingdata = read_byte(p, p);
            gpio_bit_set(GPIOB, NSS);
            gpio_bit_reset(GPIOB, NSS);
            spi_i2s_data_transmit(SPI1, convert_reg_addr(p, TRUE));
